...
### P1 - Analytics config authoring inspector  [editor]   (review H2 - the headline gap)  ✅ APPLIED (2026-06-30)
**Built + adversarially verified.** New `Editor/Analytics.Editor/LabConfigBuilder.cs` (a second `partial` of
`LabAnalyticsEditor`; the existing file gains only `partial` + one `DrawConfigBuilder()` call - all auto-detect/
...
- **In-graph (primary):** right-click a step node -> "Add Step Analytic" (creates the StepAnalytic for that step in
  the lab's config; offers to add a `LabAnalytics` to the lab root if none exists; then selects it so metrics get
  ...
  + direct config edit + SetDirty), so graph- and inspector-created StepAnalytics are identical.
...
- RENAME (Stergios 2026-06-30): the Unity-facing "Config" label is now "Analytics config" (the `[Header]` on
  `LabAnalytics.config` + the builder section label). Code keeps `LabConfig`/`config` (a fine code-level name).
...
**Done.** `LabAnalytics.Start` now pushes `config.roleCapacities -> SessionRoleSelector.SetCapacities`, so the
...
lab root's `SessionRoleSelector`. After P7 the buttons disable for roles whose config capacity is 0.
