...
**Steps (progress tracking):**
- [ ] Step 1: **Unify the menu root.** Drop the ` XR` from ALL `[AddComponentMenu]` paths (`Pi tech XR/<Module>/...`
...
- [ ] Step 2: **ORG-03:** move `SelectableTarget` / `SelectablesManager` / `MetaSelectRelay` Add-Component paths from
      the `Scenario` group -> the `Interactables` group.
- [ ] Step 3: **Mojibake:** strip the `0x85` byte from the `SceneCategories` `[MenuItem]` path (grep-verify no
...
- [ ] Step 7: **Reserve the four module slots** by the one recipe - own asmdef; optional-package deps gated via
...
- [ ] Step 8: Add the **step-fact vocabulary consts** (accelerator §H#6) - `const`s only (`step.completed` etc. +
